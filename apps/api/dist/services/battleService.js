...
class BattleService {
    ...
    async handleBattleConnection(socket, userId) {
    ...
    async startBattleCountdown(roomId) {
        try {
    ...
    async handleSubmission(socket, data) {
        try {
    ...
    async handleSpellCast(socket, data) {
        try {
    ...
    async handleReconnection(roomId, userId, socket) {
        try {
            const battle = await this.prisma.match.findUnique({
                where: { id: roomId },
                select: { player1Id: true, player2Id: true }
    ...
    async applySpellEffect(roomId, spellType, casterId, targetId) {
    ...
    startBattleTimer(roomId, timeLimit) {
    ...
    async endBattle(roomId, loserId, reason) {
        try {
            // Clear timers
            ...
            const eloChange = this.calculateEloChange(isPlayer1Winner ? battle.player1.rank : battle.player2.rank, isPlayer1Winner ? battle.player2.rank : battle.player1.rank, true);
    ...
    calculateEloChange(winnerElo, loserElo, winnerActual) {
        const K = 32;
    ...
    async updateBattleStatus(roomId, status) {
    ...
    async sendToJudge(submissionId, code, language) {
