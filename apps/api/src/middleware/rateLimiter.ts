...
import { RateLimitConfigSchema, RateLimitConfig } from '../schemas/validation';
...
export class RateLimiter {
  ...
  async checkLimit(key: string): Promise<RateLimitResult> {
    ...
    try {
      ...
      const [, currentCount] = results[1] as [string, number];
      const [, ttl] = results[2] as [string, number];
...
export function createRateLimitMiddleware(redis: RedisClientType, config: RateLimitConfig) {
...
export class TokenBucket {
  ...
  async consume(key: string, tokens: number = 1): Promise<RateLimitResult> {
    ...
    try {
      ...
      const [, bucketState] = results[0] as [string, Record<string, string>];
