...
export class SeasonalSystem {
  ...
  // Create new season
  async createSeason(config: SeasonConfig): Promise<Season> {
    try {
      // Validate dates
  ...
  async recordEloSnapshot(userId: string, battleId?: string): Promise<void> {
  ...
  async getEloHistory(userId: string): Promise<EloHistory[]> {
    if (!this.currentSeason) return [];
  ...
  // Update seasonal ELO
  async updateSeasonalElo(userId: string, newElo: number): Promise<void> {
    try {
  ...
  // Update seasonal leaderboard
  private async updateSeasonalLeaderboard(userId: string, elo: number): Promise<void> {
    if (!this.currentSeason) return;
  ...
  async getSeasonalLeaderboard(limit: number = 100): Promise<any[]> {
    if (!this.currentSeason) return [];
  ...
  async applyEloDecay(): Promise<void> {
    if (!this.currentSeason) return;
  ...
  // Clear seasonal data from Redis
  private async clearSeasonalData(seasonId: string): Promise<void> {
    const patterns = [
  ...
  private scheduleSeasonalTasks(): void {
  ...
  private async resetWeeklyLeaderboard(): Promise<void> {
    if (!this.currentSeason) return;
  ...
  private async checkSeasonEnd(): Promise<void> {
    if (!this.currentSeason) return;
  ...
  async getUserSeasonalBadges(userId: string): Promise<SeasonBadge[]> {
    try {
