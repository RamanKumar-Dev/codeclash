...
export class SocialLayerService {
  ...
  private readonly DEFAULT_CONFIG: SocialConfig = {
  ...
  async handleHeartbeat(socket: Socket): Promise<void> {
    ...
    try {
      ...
      await this.redis.setEx(
        `online:${userId}`,
        Math.ceil(this.config.heartbeatInterval / 1000) + 10, // 10 second buffer
        JSON.stringify({
          userId,
          socketId: socket.id,
          lastSeen: Date.now(),
  ...
  private async updateFriendsOnlineStatus(userId: string, isOnline: boolean): Promise<void> {
    try {
  ...
  async createPrivateRoom(
    createdBy: string,
    settings: any
  ): Promise<{ roomCode: string; room: PrivateRoom }> {
    try {
      const roomCode = this.generateRoomCode();
      const roomId = `private_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`;
  ...
  // Join private room
  async joinPrivateRoom(userId: string, roomCode: string): Promise<PrivateRoom | null> {
    try {
      const roomKey = `private_room:${roomCode}`;
      ...
      const room: PrivateRoom = JSON.parse(roomJson);
  ...
  // Leave private room
  async leavePrivateRoom(userId: string, roomCode: string): Promise<void> {
    try {
      const roomKey = `private_room:${roomCode}`;
      const roomJson = await this.redis.get(roomKey);
  ...
  private async leaveAllPrivateRooms(userId: string): Promise<void> {
    try {
  ...
  async getLiveBattles(page: number = 1, limit: number = 20): Promise<LiveBattle[]> {
    try {
  ...
  async joinBattleAsSpectator(userId: string, roomId: string): Promise<boolean> {
    try {
  ...
  async handleSpectatorChat(socket: Socket, data: { message: string; roomId: string }): Promise<void> {
    const userId = (socket as any).userId;
    const { message, roomId } = data;

    if (!userId || !message || !roomId) return;
  ...
  async featureBattle(roomId: string, featured: boolean): Promise<void> {
    try {
      const battleState = await this.redis.get(`battle:${roomId}`);
      if (!battleState) {
        throw new Error('Battle not found');
