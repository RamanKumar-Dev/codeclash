...
class BattleService {
    ...
    async handleSubmit(socket, data) {
    ...
    async executeCode(code, languageId, testCases) {
        const results = [];
        for (const testCase of testCases) {
            try {
                ...
                const response = await axios_1.default.post(`${this.judge0Url}/submissions`, submission);
    ...
    async getJudge0Result(token) {
        try {
            const response = await axios_1.default.get(`${this.judge0Url}/submissions/${token}`);
    ...
    calculateDamage(results, battleState, puzzle) {
    ...
    // End battle
    async endBattle(roomId, battleState, winnerId) {
        if (!winnerId) {
        ...
        try {
            await userService_1.UserService.updateBattleStats(winnerId, loserId, eloChange, -eloChange);
    ...
    async getPuzzleData(battleState) {
        try {
    ...
    isPlayerInBattle(userId, battleState) {
