...
export class EnhancedCodeExecutionService {
  ...
  static async executeCode(
    ...
    try {
      ...
      if (langConfig.compile) {
        try {
          await $`docker exec -i ${executionId} /bin/bash -c "cd /app && ${langConfig.compile}"`;
        } catch (error: any) {
          return {
      ...
      const actualTimeout = this.getTimeoutForPuzzle(puzzleName, timeout);
      ...
      // Cleanup Docker container and files
  ...
  private static async executeTestCase(
    ...
    try {
      // Create input file and run command
      await $`docker exec -i ${executionId} /bin/bash -c "cd /app && echo '${testCase.input}' > input_${index}.txt"`;
      await $`docker exec -i ${executionId} /bin/bash -c "cd /app && timeout ${timeout}s ${runCommand} < input_${index}.txt > output_${index}.txt"`;
      
      // Read output
      const output = await $`docker exec -i ${executionId} cat /app/output_${index}.txt`;
  ...
  // Docker container management (from Lootcode)
